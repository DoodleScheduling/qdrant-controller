...
import (
	...
	"github.com/doodlescheduling/qdrant-controller/internal/controllers"
...
const controllerName = "qdrant-controller"
...
func main() {
	...
	opts := ctrl.Options{
		...
		Cache: ctrlcache.Options{
			ByObject: map[ctrlclient.Object]ctrlcache.ByObject{
				&infrav1beta1.QdrantCluster{}: {Label: watchSelector},
			},
	...
	logger := ctrl.Log.WithName("controllers").WithName("QdrantCluster")

	QdrantClusterReconciler := &controllers.QdrantClusterReconciler{
		Client:   mgr.GetClient(),
		Log:      logger,
		Recorder: mgr.GetEventRecorderFor("QdrantCluster"),
	}

	if err = QdrantClusterReconciler.SetupWithManager(mgr, controllers.QdrantClusterReconcilerOptions{
